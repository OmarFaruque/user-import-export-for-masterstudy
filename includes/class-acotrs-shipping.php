...
class ACOTRS_Shipping extends WC_Shipping_Method{
	...
	*/
	public function acotrs_hide_other_options($rates){
		...
		if($rates){
			$hideother = array_keys(array_combine(array_keys($rates), array_column($rates, 'hide_ops')), 1);
	...
	*/
	public function init() {
			...
			$this->tax_status 							= isset($this->config['general']) && isset($this->config['general']['tax_status']) ? $this->config['general']['tax_status'] : 'taxable';
	    ...
	    private function acotrs_user_restrictions_qualified() {
			switch( $this->shipping_option_appear_for ) {
				case 'specific':
					...
					foreach( $currentUserData as $role ):
						if( array_search($role, array_column($this->ship_to_role, 'role')) )
							return true;
					endforeach;
					break;
