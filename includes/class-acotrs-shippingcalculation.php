...
if ( class_exists( 'WooCommerce' ) ) {
	...
	class ACOTRS_ShippingCalculation {
        ...
        */
			...
			if( ! empty( $cart_data ) && ! empty( $this->acotrs_method ) ) {
				...
				foreach( $this->acotrs_method->table_of_rates as $a_key => $option ) {
					...
					foreach( $option['rows'] as $r_key => $row ) {
						...
						$delivery_day = isset($row['costs'][0]['delivery_day']) && (int)$row['costs'][0]['delivery_day'] > 0 ? $row['costs'][0]['delivery_day'] : $delivery_date;
		 ...
		 * @return float
		 */
		public function acotrs_calculate_shipping_cost( $costs = array(), $cart_data ) {
			...
			foreach( $items as $item_ar ) {
				...
				if( isset( $item_ar['data'] ) && $item_ar['data']->needs_shipping() ) {
					...
					foreach($taxonomyLists as $taxny){
						...
						if($taxonomyArray){
							$taxonomys[$taxny] = !is_array($taxonomys[$taxny]) ? array() : $taxonomys[$taxny];
