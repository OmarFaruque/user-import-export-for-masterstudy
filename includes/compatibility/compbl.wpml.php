    ...
    if( !class_exists( 'ACOTRCOMPATIBILITY_Wpml' ) ){
        class ACOTRCOMPATIBILITY_Wpml{
            ...
            public function __construct(){
                ...
                add_filter('acotrs_shipping_rate_description', array($this, 'wpml_translated_shipping_discription'), 10, 1);
            ...
            */
             ...
             * @param $values array
             */
            public function wpml_settings_shipping_class_filter($values = array()){
                if( function_exists( 'icl_object_id' ) && function_exists( 'wpml_get_default_language' ) && $value ) {
                    $default_language = wpml_get_default_language();
    
                    if( is_array( $values ) && count($values) > 0 ) {
                        foreach( $values as $key => $val ) {
                            $values[ $key ] = icl_object_id( $val, 'product_shipping_class', true, $default_language );
                        }
                    } else {
                        return icl_object_id( $values, 'product_shipping_class', true, $default_language );
                    }
                }
                return $values;
            }
